...
**Verified range: `n = 2 … 3`** (kernel-computed, no `native_decide`, no new axioms).
The range is being extended (see `scripts/l5_gen.py` / `scripts/l5_remote.sh` and the
README section "Extending the range"); each additional `n` is an independent leaf
`Cbcert/FiniteLaw/N⟨n⟩.lean` added to the imports below and to the `Finset.Icc` bound.
...
range `2 ≤ n ≤ 3`, the corrected inclusion `12·d_n⁵·P_n ∈ ℤ` holds, where
`P_n = (−1)^{n+1}·p_n/C(2n,n)` is built from the project's canonical `pn`
(`Cbcert/Defs.lean`) and `d_n = lcm(1,…,n)` (`Cbcert/ErrorExhibit.lean`).

Proven by enumerating the kernel-verified per-`n` instances. -/
theorem law_upto : ∀ n ∈ Finset.Icc 2 3, CorrectedLaw n := by
  intro n hn
  fin_cases hn
  · exact law_2
  · exact law_3

/-- **Sharp nonvanishing on the verified range.** For `2 ≤ n ≤ 3`, `p_n ≠ 0`.
...
theorem pn_ne_zero_upto : ∀ n ∈ Finset.Icc 2 3, pn n ≠ 0 := by
  intro n hn
  fin_cases hn
  · exact pn_ne_zero_2
  · exact pn_ne_zero_3

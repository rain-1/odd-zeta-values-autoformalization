...
paper-level drafts, which we document. The complete development builds in
...
(Theorem~\ref{thm:erratum}), and the clean-interval theorem
(Theorem~\ref{thm:cert}) --- the part of the corrected law
\eqref{eq:corrected} that is a theorem on paper. Explicitly \emph{not}
formalized, because they are not yet theorems on paper: the full corrected
law for all $n$ (its remaining gap is a Dwork-type descent congruence, for
which extensive exact computational evidence exists, including a uniform
$\bmod\ p^5$ jet on the $a = 1$ band); and the general nonvanishing
$w_n, \wtd w_n, p_n \ne 0$. The finite-range law
$12\, d_n^5 p_n' \in \Z$ for $n \le N$ by kernel computation is routine
future work on top of the present definitions.
...
$\approx 8669$ jobs and finishes with no \lean{sorry} warnings), then
\lean{\#print axioms} on any theorem named above; each reports exactly
\lean{[propext, Classical.choice, Quot.sound]}. The repository README
...
(\lean{worthiness/cb\_certificate.tex}), 17 July 2026.

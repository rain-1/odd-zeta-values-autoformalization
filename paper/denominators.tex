...
\begin{theorem}[support; proof in the companion notes, under
independent audit]\label{thm:support}
In the totally symmetric case every prime dividing $\den(P_n)$ is at
most $2n+1$, and no prime $p\ge5$ exceeds the $\dn{n}^5$ budget.
\end{theorem}

\noindent\emph{Status.} The proof is written in the companion
computational notes and its instances are machine-verified for
$n\le6$; it will be reproduced in this paper only after a full
independent audit, and until then the reader should weight it
accordingly.

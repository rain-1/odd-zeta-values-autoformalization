...
conditional theorem, and a concrete derivation in the totally symmetric
case (where the factor $12$ appears as a doubled sine-kernel Laurent
coefficient $\tfrac16$) support this. As by-products we determine the
...
In the totally symmetric case we exhibit the factor concretely: the
excess enters through the doubled Laurent coefficient $\tfrac16$ of the
reciprocal-sine kernel, and we prove unconditionally that no prime
$p\ge5$ contributes (\S\ref{sec:symmetric}).
...
\begin{cases} 2, & p=2,\\ 1, & p=3,\\ 0, & p\ge5\ \text{(see
\S\ref{sec:anomaly})},\end{cases}
\]
i.e.\ $12\,\Dnu(\mathbf a,n)\,I'(\mathbf a\cdot n)\in\Z\zeta(5)+\Z$, and
the ceilings at $2$ and $3$ are attained.
\end{conjecture}
...
The effect is rare and non-persistent: a targeted scan of $40$ fresh
points satisfying the same trigger condition ($m_1\in\{5,7,11,13\}$
prime, $\nu_{m_1}\ge1$) produced \emph{no} further instance, and the
anomalous orbit itself relaxes to slack ($-2$ at $p=7$) at $n=2$, at
both partners coherently. We therefore state
Conjecture~\ref{conj:main}'s $p\ge5$ clause as holding up to such
isolated small-$n$, boundary-prime ($p=m_1$) fluctuations of size $1$;
characterizing them exactly is open.
...
the natural coefficient $2P$), and $2$ for $\hat P$. To our knowledge
...
most $2n+1$, and no prime $p\ge5$ exceeds the $\dn{n}^5$ budget.
\end{theorem}
...
(i) Ap\'ery $\zeta(3)$: $\dn{n}^3a_n\in\Z$ for $n\le50$ (the quoted
$2\dn{n}^3$ is slack; consistent with Rhin--Viola's exact theorem
\cite{RV3}, where the genuine $2$ rides on the $\zeta(3)$-coefficient
parity, not the denominator).
...
and the proven inclusion of \cite{KR} serve as engine validations. These
are finite verifications, not proofs; but they isolate the
Brown--Zudilin family---the only construction here that performs a
$\zeta(2)$-elimination---as the only one whose correction is attained.

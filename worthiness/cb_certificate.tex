...
\author{}
\date{17 July 2026. Revised: 19 July 2026.}
...
particular every $a_{i,j}$ is \emph{$p$-integral} for any \emph{odd} prime
$p>n$. (At $p=2$ this can fail for odd $n$, where the factor $\tfrac n2-j$ is
half-integral --- e.g.\ $a_{5,0}=\tfrac92$ at $n=1$; the theorem below only
uses $p\ge5$. The oddness hypothesis was added in the 19 July 2026 revision,
prompted by the Lean formalization of Remark~\ref{rem:lean}.) Zudilin's
...
This theorem, together with the consequent divisibility $p\mid p_n$, has been
formalized and kernel-verified in Lean~4 over Mathlib: development
\texttt{cbcert/} of this repository, writeup
\texttt{cbcert/cbcert\_formalization.tex}; zero \texttt{sorry}s, axioms
exactly \texttt{[propext, Classical.choice, Quot.sound]} on every named
theorem. The formalized statements are cast in residue form
($\mathrm{res}_p(w_n)=\mathrm{res}_p(\wt w_n)=\mathrm{res}_p(p_n)=0$), which
is faithful without any nonvanishing hypothesis; the formalization also
supplied the oddness restriction now stated before \eqref{eq:pf}.
\end{remark}
...
The conceptual formulation worth foregrounding is this. The certificate must
prescribe the $6$-jet of a polynomial at each of the $n+1$ poles --- a Hermite
interpolation problem imposing $6(n+1)$ linear conditions --- while the residue
theorem tolerates degree at most $4n+3$. Over a field of characteristic zero
the problem is overdetermined by $2n+2$ conditions and generically has no
solution; this is exactly why the characteristic-zero relations at infinity
(the $E_M$ window of \S\ref{sec:remarks}) cannot force the congruence by
themselves. In characteristic $p$ the Frobenius endomorphism supplies
$(k^p-k)^2$, which satisfies all $6(n+1)$ jet conditions \emph{simultaneously}
at degree only $2p$; the certificate then extracts the desired Laurent
coefficient at every pole at once, and the global residue theorem annihilates
their sum. In one sentence: \emph{Frobenius turns an overdetermined
characteristic-zero Hermite interpolation problem into a low-degree
interpolation certificate in characteristic $p$, and global residue duality
does the rest.} Both the exact prime window (the degree-at-infinity budget
$2p+\deg Q\le 4n+3$ computed in \S\ref{sec:remarks}, with the weight-$2$
companion fixing the endpoint $p\le 2n$) and the generalization to arbitrary
polynomial weights $Q$ (the remark closing \S\ref{sec:proof}) are immediate
from this formulation --- the mechanism is \emph{Frobenius-assisted global jet
interpolation plus residue duality}, not a bespoke identity.

\paragraph{Acknowledgment.} The Phase~2 program extending this Phase~1 theorem
to the primes $p \le n$ (the reduction to a one-step descent, the singular-gate
structure, and the $a=1$ midpoint gate) is joint work with Sol, a GPT-5.6
instance; see the companion progress note.
...
\ord_\infty G-\deg(\varphi Q)\ \ge\ 2,\qquad\text{equivalently}\quad
\deg(\varphi Q)\le\ord_\infty G-2,
\end{equation}
...
\]
Here $\ord_\infty\overline R_n=4n+5$ and $\deg\varphi=2p$, so the degree budget
\eqref{eq:degbudget} reads $\ord_\infty\overline R_n-\deg\varphi=(4n+5)-2p\ge2$,
i.e. $p\le2n+1$; since $p\le2n$ this holds with room to spare and
$\varphi\overline R_n=O(k^{\,2p-4n-5})=O(k^{-5})$. Hence
$\Res_\infty(\varphi\overline R_n)=0$ and $w_n\equiv0\pmod p$.
...
\]
Now $\deg(\varphi Q)=2p+2$, so \eqref{eq:degbudget} reads
$\ord_\infty\overline R_n-\deg(\varphi Q)=(4n+5)-(2p+2)=4n+3-2p\ge2$, i.e.
$p\le2n$. Under our hypothesis $p\le2n$ this holds and
$\varphi Q\overline R_n=O(k^{-3})$, so again the residue at infinity vanishes
...
\end{itemize}
\emph{Observation (not needed for the theorem).} The upper bound appears sharp
for the method: in exact linear-algebra checks for $p>2n+1$ no polynomial of
degree $\le 4n+3$ meets \eqref{eq:jet} (the Hermite system is inconsistent
mod $p$), and $w_n$ is there generically a unit. We record this as an
observation, verified on small ranges rather than proved; it is not used in
Theorem~\ref{thm:main}, which requires only the \emph{existence} of the
certificate on $n<p\le 2n$, supplied by Lemma~\ref{lem:frob}.

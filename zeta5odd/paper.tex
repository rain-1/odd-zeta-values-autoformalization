...
and, after Ap\'ery, for $s=3$ --- the latter recently formalized in Lean~4 by
Liu, Zhang and Zhi~\cite{LiuZhangZhi2025} via Beukers' integral method; for odd
$s\ge5$ only partial results are known.
...
external \emph{PrimeNumberTheoremAnd} project does supply the required limit
$d_n^{1/n}\to e$ --- indeed the Lean~4 formalization of $\zeta(3)$ by Liu, Zhang
and Zhi~\cite{LiuZhangZhi2025} obtains its $d_n$ bound this way, from the
elementary $d_n\le n^{\pi(n)}$ together with $\pi(n)\log n\sim n$ --- but adding
it here would pin an incompatible Mathlib revision, a dependency update we chose
to avoid in order to keep the development self-contained over a single Mathlib.
We therefore follow Zudilin's own remark (\cite{Zudilin2018}, \S4): using the
...
\bibitem{LiuZhangZhi2025}
  J.~Liu, J.~Zhang, L.~Zhi,
  \emph{A formal proof of the irrationality of $\zeta(3)$ in Lean~4},
  arXiv:2503.07625 (2025);
  code: \url{https://github.com/ahhwuhu/zeta_3_irrational}.
\end{thebibliography}
